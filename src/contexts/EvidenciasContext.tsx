...
export function EvidenciasProvider({ children }: { children: ReactNode }) {
  ...
  const load = async () => {
    setLoading(true);
    const data = await fetchAll("evidencias");
  ...
  return (
    <EvidenciasContext.Provider value={{ evidencias, loading, addEvidencia, updateEvidencia, deleteEvidencia, refresh: load, uploadAnexo }}>
      {children}
