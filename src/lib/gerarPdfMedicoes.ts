...
export function gerarPdfMedicoes(medicoes: MedicaoServico[]): jsPDF {
  ...
  autoTable(doc, {
    startY: y,
    head: [["Nº", "Cliente / Obra", "Fornecedor", "Contrato", "Descrição", "Status", "Contratado", "Medido", "% Exec."]],
    body: medicoes.map((m) => [
      m.numero.toString(),
      m.cliente_nome || "",
    ...
    headStyles: { fillColor: [30, 58, 107], textColor: [255, 255, 255], fontStyle: "bold" },
    columnStyles: { 0: { cellWidth: 12 }, 4: { cellWidth: 50 } },
    margin: { left: 14, right: 14 },
  });
...
export function downloadPdfMedicoes(medicoes: MedicaoServico[]) {
  const doc = gerarPdfMedicoes(medicoes);
